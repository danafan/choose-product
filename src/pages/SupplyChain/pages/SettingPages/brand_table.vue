			...
			<el-table size="mini" :data="data" tooltip-effect="dark" style="width: 100%" :header-cell-style="{'background':'#f4f4f4','text-align': 'center'}" :cell-style="{'text-align':'center'}" :max-height="max_height" v-loading="loading">
				...
				<el-table-column label="操作" width="180" fixed="right">
					<template slot-scope="scope">
						...
						<el-button type="text" size="small" @click="deleteFn(scope.row.id)" v-if="button_list.del == 1">删除</el-button>
					</template>
				</el-table-column>
			</el-table>
			...
			<div class="dialog_content">
				<el-form size="small" label-width="80px">
					<el-form-item label="品牌：" required>
						<el-input type="textarea" style="width: 220px" autosize placeholder="请输入品牌（多个请换行）" v-model="name">
						</el-input>
					</el-form-item>
				</el-form>
			</div>
	...
	export default{
    	...
    	methods: {
			...
			getData(){
				...
				this.loading = true;
				resource.brandList(arg).then(res => {
					if(res.data.code == 1){
						this.loading = false;
			...
			deleteFn(id){
				this.$confirm('确认删除该品牌?', '提示', {
					...
					resource.delBrand(arg).then(res => {
						if(res.data.code == 1){
			...
			addFn(type,item){
				this.type = type;
				this.id = type == '1'?'':item.id;
				this.name = type == '1'?'':item.name;
				this.dialog_title = type == '1'?'添加品牌':'编辑品牌';
			...
			commitFn(){
				...
				if(this.type == '1'){		//添加品牌
					resource.addBrand(arg).then(res => {
						if(res.data.code == 1){
							this.$message.success(res.data.msg);
					...
					arg.id = this.id;
					resource.editBrand(arg).then(res => {
						if(res.data.code == 1){
							this.$message.success(res.data.msg);
							this.show_dialog = false;
							//获取品牌列表
