	...
	<div class="setting_content">
		<el-card class="card_box" id="card_box">
			...
			<el-table size="mini" :data="data" tooltip-effect="dark" style="width: 100%" :header-cell-style="{'background':'#f4f4f4','text-align': 'center'}" :cell-style="{'text-align':'center'}" :max-height="max_height" v-loading="loading">
				<el-table-column label="类目" prop="category_name"></el-table-column>
				...
				<el-table-column label="操作" width="180" fixed="right">
					<template slot-scope="scope">
						...
						<el-button type="text" size="small" @click="deleteFn(scope.row.category_id)" v-if="button_list.del == 1">删除</el-button>
					</template>
				</el-table-column>
			</el-table>
			...
			<div class="dialog_content">
				<el-form size="small" label-width="100px">
					<el-form-item label="类目名称：" required>
						<el-input type="textarea" style="width: 220px" autosize placeholder="请输入类目名称（多个请换行）" v-model="name">
						</el-input>
					</el-form-item>
				</el-form>
			</div>
	...
	export default{
    	...
    	methods: {
			...
			getData(){
				...
				this.loading = true;
				resource.getCateList(arg).then(res => {
					if(res.data.code == 1){
						this.loading = false;
			...
			deleteFn(id){
				this.$confirm('确认删除该类目?', '提示', {
					...
					resource.deleteCate(arg).then(res => {
						if(res.data.code == 1){
			...
			addFn(type,item){
				this.type = type;
				this.id = type == '1'?'':item.category_id;
				this.name = type == '1'?'':item.category_name;
				this.dialog_title = type == '1'?'添加类目':'编辑类目';
